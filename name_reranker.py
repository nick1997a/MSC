...
def reformat_test_datasamples(correct_dict,test_data_path):
    mention_rela = []
    ...
    #     for line in file2:
    #         mention_rela.append(json.loads(line))
    with open(test_data_path, 'r',encoding='utf-8') as file3:
        for line in file3:
            mention_rela.append(json.loads(line))
...
def test(model, val_loader, loss_fn, device):
    ...
    with torch.no_grad():
        for X_inp_id, _, X_atte_mask, Y_val in tqdm(val_loader):
            #X_inp_id = X_inp_id.squeeze(dim=1)
            #X_atte_mask = X_atte_mask.squeeze(dim=1)
...
def onlyname_acck( model_name, alldata,all_mention_embeddings, entities_embeddings,umls_entinames,retri_k,acc_k,trained_crossencoder):
    ...
    our_tokenizer.add_tokens(['[E]', '[/E]'])
    #our_model = CrossEncoder(model_name, 1)
    our_model = CrossEncoder(model_name, 1).to(device)
    our_model.load_state_dict(torch.load(trained_crossencoder))

    file_path = 'retrieved_topK_results.json'
    with open(file_path,'r') as f:
        all_retrieved_topK = json.load(f)

    it_keys = list(all_retrieved_topK.keys())
    num = retri_k
    accuracy = []
    for i in tqdm(range(0,len(all_retrieved_topK))):
        correct_umls_name = alldata[i]['entity_description']
        retrieved_topK_names = all_retrieved_topK[it_keys[i]]['terms'][:num]
        retrieved_topK_indices = all_retrieved_topK[it_keys[i]]['indices'][:num]

        if correct_umls_name in retrieved_topK_names[:acc_k]:
            accuracy.append(1)
        else:
            rerank_pairs = [correct_umls_name + '[SEP]' + b for b in retrieved_topK_names]
            x_features, y_features = Generate_features(our_tokenizer, rerank_pairs, 50)
            ...
            probs = test(our_model, test_dataloader,loss_fn,device)


            result_dict = {}
            for j in range(0,len(probs)):
                result_dict[retrieved_topK_indices[j]] = probs[j]
            ...
            second_indice_info = [umls_entinames[k] for k in ranked_indices[:acc_k]]

            if correct_umls_name in second_indice_info:
                accuracy.append(1)
            else:
                accuracy.append(0)
...
def main():
    parser = argparse.ArgumentParser(description="Test different parameters from shell")
    parser.add_argument('--tokenizer', type=str, default='cambridgeltl/SapBERT-from-PubMedBERT-fulltext', help="")
    parser.add_argument('--umls_path', type=str, default='./Exact_umls/', help="2017 umls AA")
    parser.add_argument('--testdata_path', type=str, default='/home/gan/CODES/CrossEncoder/load_umls/tra_val_test/mentions/test.json', help="")
    parser.add_argument('--trained_crossencoder',type=str, default='./crossencoder/best_model_bt52.pth')
    parser.add_argument('--umls_vector_mode', type=str,default='sapbert')
    parser.add_argument('--retri_K',type=int,default=15)
    parser.add_argument('--acc_K',type=int,default=5)
    parser.add_argument('--output_file',type=str,default='')

    args = parser.parse_args()
    output_file = f"output/retri_{args.retri_K}_acc_{args.acc_K}_{args.trained_crossencoder.split('/')[-1]}.txt"
    ...
    print('umls entities loaded')

    tok_nizer = AutoTokenizer.from_pretrained(args.tokenizer)
    sapbert_model = AutoModel.from_pretrained(args.tokenizer).cuda()
    print('tokenizer name:',args.tokenizer,'\n')


    test_dataset = reformat_test_datasamples(cui_to_entityinfo,args.testdata_path)
    print('test dataset path:',args.testdata_path)
    print('testdata length:',len(test_dataset),'\n')


    enti_vectors = np.load(args.umls_path+'umls_name_vectors_'+args.umls_vector_mode+'.npy')
    print('umls vector:',args.umls_vector_mode)
    print('umls vector shape:',enti_vectors.shape,'\n')

    print('compute acc@', args.acc_K, '(retrieve ', args.retri_K, ' candidates)')
    print('reranker model used:',args.trained_crossencoder)
    qu_vectors = generate_query_vectors(test_dataset,tok_nizer, sapbert_model)

    onlyname_acck(args.tokenizer,test_dataset,qu_vectors, enti_vectors, umls_entity_names,args.retri_K,args.acc_K, args.trained_crossencoder)
    sys.stdout.close()
