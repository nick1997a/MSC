...
def compute_acck( alldata,all_mention_embeddings, entities_embeddings,umls_entinames):
    ...
    model = CrossEncoder('microsoft/BiomedNLP-BiomedBERT-base-uncased-abstract-fulltext', 1).to(device)
    ...
    for i in tqdm(range(0, len(query_vectors))):
        ...
        KKK = np.squeeze(indices)
        indice_info = [umls_entinames[i] for i in KKK[:1]]
        if correct in indice_info:
            acc = 1
        else:
            os.environ["TOKENIZERS_PARALLELISM"] = "false"
            retre_names = [umls_entinames[i] for i in KKK]
            ...
            for i in range(0,len(probs)):
                result_dict[list(KKK)[i]] = probs[i]
            ...
            if correct in second_indice_info:
                acc = 1
            else:
                acc = 0
...
def main():
    path = '/home/gan/CODES/CrossEncoder/AAANEW/'
    umls = load_umls(path)
    ...
    # np.save('umls_name_vectors_sapbert.npy', enti_vectors)
    #
